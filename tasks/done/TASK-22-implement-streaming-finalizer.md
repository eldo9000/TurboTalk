...
## Post-landing evidence — 2026-05-03

Re-ran `tasks/done/timing-protocol.md` on the same host (arm64 / macOS 26.4.1)
after commit `3d13660`:

| | post-release `total=` (ms) | TASK-21 baseline (ms) | speedup |
|---|---|---|---|
| Short (1.41s after VAD) | **49.60** | 192.49 | 3.9× |
| Long  (8.13s after VAD) | **61.94** | 781.94 | 12.6× |

Both clear the 250 ms gate; both also clear the 100 ms aspirational target.

Long-recording log line (verbatim):
```
[audio] stage timings (ms): capture_clone=21.74 incremental_resample_total=855.37 incremental_vad_total=291.87 finalize_flush=3.69 streaming_finish=3.81 wav_write=7.25 total=61.94 (streaming, vad_frames=319, resampled_total=152789)
```

The `incremental_resample_total` (855 ms) is higher than the old batch
resample (654 ms) — chunked `rubato::process` has per-call overhead. That's
fine: it runs concurrently with recording, off the user-visible critical
path. The post-release `total=` field is what the user actually waits for.

Quality check: no word clipping observed on either recording.
- Short transcript: "All right, test, test." — leading "All right" preserved.
- Long transcript: "All right, doing a longer testing recording, so we'll
  see if this works actually correct." — both ends clean.

Whisper latency unchanged from TASK-21 baseline (cold 3102 ms, warm 2093 ms),
confirming TASK-20's option-3 model-warmup-still-pending status holds.

Verdict: TASK-22 success signal observed end-to-end. Streaming finalizer
shipped.
