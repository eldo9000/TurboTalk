...
**Backend selector — landed 2026-05-23 (TASK-60):** `BackendFamily` enum (Whisper/Moonshine/Parakeet) added to `settings.rs`; persists as `backend = "whisper"` in `config.toml`. `build_backend()` reads `cfg.backend` — `TT_BACKEND` env var removed. Moonshine/Parakeet fall back to Whisper with a warning when feature flags are off. `list_models_for_family` command added. Settings tab has a Transcription Engine picker. Onboarding Step 4 shows engine picker before model list.

**Parakeet backend scaffold — landed 2026-05-23 (TASK-59):** `ParakeetBackend` struct, `TranscriptionBackend` impl, path validation, and `download_parakeet_model` tauri command are in place. Backend family routed via `cfg.backend = Parakeet` (TASK-60). `parakeet = []` feature in Cargo.toml. Currently a documented stub — same ort conflict blocker as Moonshine.

**Moonshine backend scaffold — landed 2026-05-23 (TASK-58):** `MoonshineBackend` struct, `TranscriptionBackend` impl, path validation, and `download_moonshine_model` tauri command are in place. Backend family routed via `cfg.backend = Moonshine` (TASK-60). ONNX runtime dylib placeholder committed, bundle.resources wired. Currently a documented stub — live inference blocked by `ort` version conflict between `transcribe-rs` (rc.12) and `vad-rs` (rc.9).
