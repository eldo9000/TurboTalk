...
**Parakeet backend — landed 2026-05-23 (TASK-59), download wired 2026-05-26:** `ParakeetBackend` struct, `TranscriptionBackend` impl, path validation, and `download_parakeet_model` tauri command are in place. Backend family routed via `cfg.backend = Parakeet` (TASK-60). Download URLs fixed to `istupakov/parakeet-tdt-0.6b-v2-onnx` int8 bundle. End-to-end dictation not yet user-confirmed.
