...
**Current state:** macOS dictation is feature-complete for personal use. All three backends proven end-to-end: Whisper (multilingual), Moonshine (English, FP32 ONNX), Parakeet (English/multilingual, int8 ONNX). Parakeet is the default for new installs. Whisper Silero VAD pre-filter user-confirmed 2026-05-26 (bundled `ggml-silero-v5.1.2.bin`). Latest commit `1a41878`: Parakeet v3 catalog, chunk WAV fix, variant-switch UI fix, Whisper-style model naming. Uncommitted local work: SESSION cleanup, VAD model bundle, CoreML dyld guards.

**Next action:** None pinned. CoreML phase 2 remains optional sidecar only (see `docs/reference/COREML-BLOCKER.md`). Windows v1.0 path: TASK-25/26 (real hotkey + paste on hardware).
