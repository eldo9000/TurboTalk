...
**Current state:** TASK-44 complete. audio_ctx=512 benched at ~63% faster than default across all utterance lengths (short 1178→410ms, medium 1307→532ms, long 1395→543ms). 768 had an unexplained regression on the 8s utterance (2033ms); 512 was clean everywhere. Applied as permanent default. TASK-42 benched: q5_0 quality OK, speedup insufficient, large-v3-turbo stays default. TASK-46 done: post-stop sleep 25→10ms (transcribe.rs poll already gone via TASK-47). Next: TASK-45 (thread count bench).

Previous state: TASK-42 benched. q5_0 (~547 MB) passes quality vs large-v3-turbo (~1.5 GB) but speedup not material enough to promote. large-v3-turbo remains default. q5_0 documented as fast-mode candidate for future "quality vs speed" toggle. TASK-46 also complete: audio.rs post-stop sleep reduced 25 ms → 10 ms (transcribe.rs poll already gone — TASK-47 eliminated per-call whisper-cli). Next: TASK-44 (--audio-ctx bench) or TASK-45 (thread count bench).
...
**Next action:** Confirm warm-worker proof end-to-end (second dictation faster than first, `[transcribe] worker ready` in log). Then TASK-42 (q5_0 bench), TASK-44 (audio-ctx), TASK-45 (threads), TASK-46 (shave sleeps).
