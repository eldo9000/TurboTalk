...
**Current state:** TASK-58 landed (partial) — `MoonshineBackend` struct, `TranscriptionBackend` impl, and all path/download infrastructure are in place. `build_backend()` reads `TT_BACKEND` env var and routes to Moonshine when set. `download_moonshine_model` tauri command exists with the progress-event pattern. `libonnxruntime.dylib` placeholder committed; bundle.resources wired in `tauri.macos.conf.json`. 97/97 tests green, clippy clean.

BLOCKER: `transcribe-rs 0.3.11` (required for live Moonshine inference) pins `ort = "=2.0.0-rc.12"`. `vad-rs 0.1.5` pins `ort = "=2.0.0-rc.9"`. Cargo cannot resolve two exact-pinned versions of `ort` in one build — even when `transcribe-rs` is optional. Until vad-rs updates its ort pin (or VAD is replaced), the Moonshine backend is a documented stub that returns a clear error at construction time. Unblock paths documented in `src-tauri/src/transcribe_backends/moonshine.rs` and `src-tauri/Cargo.toml`. The `moonshine` feature flag is declared; once the dep is resolvable, uncomment `transcribe-rs` in Cargo.toml and the impl becomes live.

Previous state: TASK-57 landed — `TranscriptionBackend` trait extracted; `WhisperBackend` is the sole concrete impl. `WORKER` static is now `Mutex<Option<Arc<dyn TranscriptionBackend>>>`. `worker_for` compares `model_identity()` strings instead of `&Path`s. `TranscriptionWorker` is a deprecated type alias kept for the one test that uses a struct literal. No behavior change. 97/97 tests green, `cargo clippy -D warnings` clean.

Previous state: TASK-56 landed — Silero VAD pre-filter for whisper-server. `whisper.vad_enabled` (default true) added to settings; when enabled, `--vad --vad-model ggml-silero-v5.1.2.bin` are passed to the whisper-server spawn so silent regions are skipped before the decoder runs. Settings toggle added to Modes tab → Whisper section. VAD model file is a 1-byte placeholder in `src-tauri/binaries/` — real model (`ggml-silero-v5.1.2.bin`, ~2 MB) must be downloaded from `https://huggingface.co/ggml-org/whisper-vad/resolve/main/ggml-silero-v5.1.2.bin` and placed there (network was unavailable in this session). If the model file is absent, server starts without VAD (graceful fallback, warning logged). 97/97 tests green.

Previous state: TASK-55 landed — hallucination detection filter. Three signals (gzip compression ratio, trigram repetition, non-letter ratio) detect Whisper garbage on silence; rejected transcripts are displayed in the main window with a "⚠ filtered" badge and a toast, paste skipped. 97/97 tests green.

Previous state: v0.9.0 released. First confidently-shareable macOS release — no known bugs for Mac users. Direct-download README badge live (`TurboTalk-macOS-arm64.dmg` stable asset added to CI). macOS feature development complete. Next milestone: Windows/Linux porting (TASK-25/26) → v1.0. Bug fixes from this session landed: (1) onboarding bounce during model download on window focus; (2) silent recording rejection with no feedback (tail-empty segment-lost bug — all audio in segments, tail 0 samples); (3) Whisper hallucination on `speech_detected=false` — gated tail Whisper call on `speech_detected` propagated through `StopOutcome::Wav`; (4) overlay stuck in recording mode during seg-recovery's blocking Whisper wait — emit `ptt-up` before `join_segments()`; (5) new recording UI corrupted by delayed seg-recovery events — bail if `CURRENT_JOB_ID` set after `join_segments()` returns. Model lineup updated: `ggml-large-v3-turbo` (1.6 GB) promoted to Recommended; `ggml-large-v3-turbo-q5_0` (574 MB) demoted to "Small / low RAM" fallback; `ggml-large-v3` (3.1 GB) restored as "Large / high accuracy, high RAM, slow". Model card typography redesigned: tier name + model name pill inline, description on own line below.

Previous state (2026-05-20): Length indicator redesigned (TASK-57). The lorem ipsum word pills were replaced with a lines/paragraphs counter badge positioned to the right of the recording overlay pill. The badge shows `— / N lines / N paragraphs` derived from VAD-estimated word count. Unit is user-configurable in Settings (Lines default, Paragraphs option). Feature is gated by the existing `transcript_size_indicator` config flag; a new `length_indicator_unit` config field was added.
