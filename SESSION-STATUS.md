...
**Current state:** Two regressions fixed, dictation back to snappy state (commit `3871090`).
(1) Process leak / Mac freeze: persistent worker in `static WORKER` never runs Drop on process exit, so setsid'd whisper-server child survived every quit (10 orphans found, 5 holding 1.6 GB each). Fixed via `.build()?.run(cb)` split in lib.rs — `RunEvent::Exit` now calls `transcribe::abort_active()`.
(2) Triple-paste / repetition hallucination: TASK-47 (whisper-cli → whisper-server) lost decoding flags from commit `55cfa21`. whisper-server defaults — `temperature_inc=0.2` (fallback retry), `suppress_nst=false`, `beam_size=-1` (greedy) — produce "same phrase 3x" on short/silent audio. Fixed in transcribe.rs HTTP POST: temperature=0, temperature_inc=0, suppress_nst=true, no_context=true, beam_size=5, plus the stored vocabulary now wired into the `prompt` form field.
