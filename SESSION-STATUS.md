...
**Current state:** TASK-48 phase 1 complete (commit `e81fb11`). CoreML-enabled whisper-server built from whisper.cpp v1.8.4 source and committed to `src-tauri/binaries/`. Build blocker solved: cmake `check_cxx_source_runs` hangs for ARM SVE on Apple M4 (no SVE support); patched to `check_cxx_source_compiles` in `ggml/src/ggml-cpu/CMakeLists.txt`. CoreML chain confirmed: whisper-server → libwhisper.1.dylib → libwhisper.coreml.dylib → CoreML.framework. rpath is `@loader_path` on all binaries — dev mode works. New dylibs committed: `libwhisper.coreml.dylib`, `libggml-cpu.0.dylib`, `libggml-blas.0.dylib`, `libggml-metal.0.dylib`. Old `.so` files (`libggml-blas.so`, `libggml-metal.so`) still in binaries/ but unused by new binary. CoreML is compile-time only — no runtime flag. Binary auto-loads `.mlmodelc` from same dir as `.bin` model if present.
...
**Next action (TASK-48 phase 2):**
1. `tauri.conf.json` — add `bundle.resources` for the new dylibs so they ship in the packaged DMG: `libwhisper.coreml.dylib`, `libggml-cpu.0.dylib`, `libggml-blas.0.dylib`, `libggml-metal.0.dylib`. Remove stale `libggml-blas.so` and `libggml-metal.so` from `src-tauri/binaries/` (unused by new binary).
2. Download `ggml-large-v3-turbo-encoder.mlmodelc.zip` (1.17 GB) from `https://huggingface.co/ggerganov/whisper.cpp/resolve/main/ggml-large-v3-turbo-encoder.mlmodelc.zip` → extract alongside the model `.bin` in `~/.config/librewin/turbotalk/models/`.
3. Test CoreML activation: start the app, dictate, confirm `/tmp/whisper-server-stderr.log` shows `whisper_init_state: loading Core ML model from ...` followed by success.
4. Bench: 5+ utterances of varied length, capture `[transcribe] whisper took N ms` for each. Compare CoreML-active vs Metal-active medians against the established baseline (short≈410ms, medium≈532ms, long≈543ms).
5. Fallback test: rename the `.mlmodelc` dir, confirm dictation still works via Metal.
6. Ship/defer decision: >20% median improvement and clean install → ship; <10% or fragile → defer + revert.
