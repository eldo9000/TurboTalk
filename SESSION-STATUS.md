...
**Current state:** Long-recording failure diagnosed and the immediate bug fixed. A ~3 min dictation (job 251, 175 s of dense speech) failed at exactly 30.047 s with `"error sending request"`. Root cause: reqwest's **blocking** client defaults to a 30 s total request timeout (`Timeout::default() = Some(30 s)` — the async client defaults to `None`); `transcribe.rs` used `Client::new()` and inherited it silently, so a transcription that ran >30 s got its connection cut mid-inference while whisper-server kept running. whisper-server is innocent — live-server tests: 180 s WAV → 200 in 23.8 s, 480 s WAV (15.3 MB) → 200 in 55.2 s. **Fix shipped (uncommitted):** explicit 120 s timeout on the transcribe client (`transcribe.rs` ~line 392); `cargo check` green. **Plan filed:** `tasks/TASK-54-streaming-chunked-transcription.md` (Option 2 — segment-on-silence streaming transcription, the UX upgrade) for review before implementation. Two companion overlay features requested (elapsed-time ticker, flash-to-warn past a threshold) — to be filed as small separate tasks.
