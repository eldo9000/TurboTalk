...
<div class="tt-history flex-1 min-h-0 flex flex-col">
  {#if state.transcriptError}
    <div class="tt-banner-error">
      <span class="tt-banner-error-msg">{state.transcriptError}</span>
  ...
  {#if state.history.length === 0}
    <div class="tt-history-empty">
      {#if state.recording || state.transcribing}
        <p class="tt-history-empty-status">{state.recording ? 'Recording…' : 'Transcribing…'}</p>
      {:else}
        <kbd class="tt-kbd">{state.hotkeyLabel}</kbd>
        <p class="tt-history-empty-hint">
          {state.cfgHotkeyMode === 'toggle' ? 'Press to start · press again to stop' : 'Hold to record'}
        </p>
      {/if}
    </div>
  {:else}
    <div class="tt-history-list">
      {#each state.history as item (item.ts)}
        <button
        ...
        >
          <span class="tt-history-text" class:tt-history-text-hidden={state.copiedTs === item.ts}>
            {item.text}
          </span>
          {#if state.copiedTs === item.ts}
  ...
  <div class="tt-history-actions">
    <button
      ...
      class:tt-btn-recording={state.recording}
    >
